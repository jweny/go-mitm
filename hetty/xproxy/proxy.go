...
func (p *Proxy) handleConnect(w http.ResponseWriter) {
	...
	if !ok {
		p.logger.Errorf("ResponseWriter is not a http.Hijacker.",
	...
	if err != nil {
		p.logger.Errorf("Hijacking client connection failed.",
			"error", err)
		writeError(w, http.StatusServiceUnavailable)
	...
	if err != nil {
		p.logger.Errorf("Securing client connection failed.",
			"error", err,
	...
	if err != nil && !errors.Is(err, ErrAlreadyAccepted) {
		p.logger.Errorf("Serving HTTP request failed.",
			"error", err)
	}
...
func (p *Proxy) errorHandler(w http.ResponseWriter, r *http.Request, err error) {
	...
	case !errors.Is(err, context.Canceled):
		p.logger.Errorf("Failed to proxy request.",
			"error", err)
	case errors.Is(err, context.Canceled):
		p.logger.Errorf("Proxy request was cancelled.")
	}
